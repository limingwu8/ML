...
# read dataset information from pickle
pkl_file = open('dataset_info.pkl', 'rb')
X, Y, X_test, sizes_test, train_ids, test_ids, \
X_train_contour, Y_train_contour, contour = pickle.load(pkl_file)
pkl_file.close()

# to make shape as (batch_size, channel, width, height)
...
print(sizes_test[15])
print('X_train_contour.shape: ' + str(X_train_contour.shape))
print('Y_train_contour.shape: ' + str(Y_train_contour.shape))
print('contour.shape: ' + str(contour.shape))
# X = np.random.randn(570,3,256,256)
# Y = np.random.randn(570,1)
...
train_loader = DataLoader(train_set, batch_size=24, shuffle=True, **kwargs)
val_loader = DataLoader(val_set, batch_size=24, shuffle=True, **kwargs)

# split training set to training set and validation set
# X_train, X_val, Y_train, Y_val = train_test_split(X, Y, test_size=0.33, random_state=42)
#
# train = TensorDataset(torch.FloatTensor(X_train), torch.from_numpy(Y_train.astype(np.float32)))
# val = TensorDataset(torch.FloatTensor(X_val), torch.from_numpy(Y_val.astype(np.float32)))
#
# # train = TensorDataset(torch.FloatTensor(X_train), torch.from_numpy(Y_train))
# kwargs = {'num_workers': 1, 'pin_memory': True}
# train_loader = DataLoader(train, batch_size=24, shuffle=True, **kwargs)
# val_loader = DataLoader(val, batch_size=24, shuffle=False, **kwargs)
...
def train(epoch):
    ...
    for batch_idx, (data, target) in enumerate(train_loader):
        ...
        loss.backward()
        optimizer.step()
        print('epoch: ' + str(epoch) + ', batch: ' + str(batch_idx) + ', loss: ' + str(loss.data[0]))
        log.write('epoch: ' + str(epoch) + ', batch: ' + str(batch_idx) + ', loss: ' + str(loss.data[0]) + '\n')
        # print(batch_idx)
    log.close()
def val():
    model.eval()
    ...
    log.write('************validation loss: ' + str(test_loss) + ' ************\n')
    log.close()
