...
def train(model, dataloader, opt, criterion, epoch):
    ...
    with open('logs.txt', 'a') as file:
        for batch_idx, sample_batched in enumerate(dataloader):
            ...
            optimizer.zero_grad()
            output = model(data)
            # output = (output > 0.5).type(config.DTYPE)
...
def val(model, dataloader, opt, criterion):
    model.eval()
    num_batches = 0
    avg_loss = 0
    with open('logs.txt', 'a') as file:
...
if __name__ == '__main__':
    ...
    model = UNet(input_channels=3, nclasses=1).cuda()
