...
def map_masks_to_images(images, labels):
    ...
    return np.asarray(contour_images), np.asarray(contour_labels), np.asarray(contours)

def plot_images(images, labels, contour_images, contour_labels, contours, index = -1):
    if index == -1:
        index = np.random.randint(images.shape[0])
    image = images[index]
    label = labels[index].reshape(IMG_W, IMG_H)
    contour_image = contour_images[index]
    contour_label = contour_labels[index].reshape(IMG_W, IMG_H)
    contour = contours[index].reshape(IMG_W, IMG_H)
    imgs = [image,label,contour_image,contour_label, contour]
    titles = ['image','label','contour image', 'contour label','contour']
...
def read_files(file_dir):
    ...
    return images, labels, img_dims, imageId

# copy from https://www.kaggle.com/rakhlin/fast-run-length-encoding-python
def rle_encoding(x):
    '''
...
if __name__ == '__main__':
    ...
    images, labels, img_dims, imageId = read_files(file_dir)
    contour_images, contour_labels, contours = map_masks_to_images(images, labels)
    plot_images(images, labels, contour_images, contour_labels, contours)
    # input_path = '/root/dataset/dataScienceBowl2018/stage1_train'
